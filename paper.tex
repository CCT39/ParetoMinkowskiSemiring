...
\usepackage[
    ...
    linkcolor=MikuCyan!85!black, % 內部連結
    citecolor=MikuCyan!85!black, % 參考文獻連結
    urlcolor=MikuCyan!85!black, % 外部網址連結
    filecolor=MikuCyan!85!black, % 本地檔案連結
...
The author thanks the anonymous reviewers for their comments, and the \textit{Project DIVA Arcade} community for decades of empirical research into what this paper calls ``constrained bi-objective optimization on a layered DAG,'' a phrase none of them would ever use voluntarily. Further thanks are due to the SEGA engineers for inadvertently turning a pop-culture rhythm game into a rigorous exercise in path algebra, and to the Vocaloid producers (\textit{Vocaloid-Ps}) whose relentless BPMs necessitated these computationally cursed charts.

Special thanks are owed to Hatsune Miku, age 16 forever, who (despite being a virtual character with no legal personhood, no university affiliation, and no h-index) is arguably the principal investigator of this entire line of research. Without her vocal synthesizer lending its name to the game, there would be no Shared Release Constraint, no bi-objective scoring, no Pareto-Minkowski semiring, and certainly no excuse to name an algebraic structure after a \texttt{\#39c5bb}-haired pop idol.

Any errors in this paper are, of course, entirely the author's own. Miku's performances, unlike our proofs, have never required correction.
